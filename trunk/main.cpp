...
void analyze_results(FILE* out)
{
	...
	for ( int i = 0; i < cpu_index; i++ )
	{
		...
		fprintf(out, "\niii)%28s:   %25g | %25g | %7.3f\% \n", "Early Exercise Value", exv_cpu, exv_gpu,100.0*( exv_cpu - exv_gpu )/exv_cpu );
		
		fprintf(out, "\niv) %-28s:   %25s   %25s   %7s \n", "Resource Usage", "", "", "Ratio");
		//fprintf(out, "\niv)Resource Usage:\n");
		
		fprintf(out, "%32s:   %25g | %25g | %7.2fx \n", "Total time taken (sec)", r_sets[cpu_index]->net_clock_time, r_sets[i]->net_clock_time ,
		r_sets[cpu_index]->net_clock_time/r_sets[i]->net_clock_time);
		
		fprintf(out, "%32s:   %25g | %25g | %7.2f\% \n", "Peak memory estimate (MB)", r_sets[cpu_index]->memory_usage, r_sets[i]->memory_usage ,
		100.0*r_sets[i]->memory_usage/r_sets[cpu_index]->memory_usage );
	}
	fprintf(out, "\nNOTE:\n-------\n\t- When comparing total time taken, Ratio(\%) represents the speed-up of GPU over CPU");
	fprintf(out, "\n\t- When comparing memory usage, Ratio(\%) represents the percentage of memory used by GPU as compared to CPU\n");
}
...
int main(int argc, char** argv )
{
    ...
    if ( nv1 )
    {
    	...
    	fprintf(stderr, "	* Completed %s in %g secs.\n", (char*)r_set->desc.c_str(), r_set->net_clock_time);
    }
